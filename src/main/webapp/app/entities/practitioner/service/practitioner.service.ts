...
type RestOf<T extends IPractitioner | NewPractitioner> = Omit<T, 'dob'> & {
  dob?: string | null;
};
...
export class PractitionerService {
  ...
  protected convertDateFromClient<T extends IPractitioner | NewPractitioner | PartialUpdatePractitioner>(practitioner: T): RestOf<T> {
    return {
      ...practitioner,
      dob: practitioner.dob?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPractitioner: RestPractitioner): IPractitioner {
    return {
      ...restPractitioner,
      dob: restPractitioner.dob ? dayjs(restPractitioner.dob) : undefined,
    };
  }
