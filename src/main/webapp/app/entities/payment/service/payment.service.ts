...
type RestOf<T extends IPayment | NewPayment> = Omit<T, 'datePaid'> & {
...
export class PaymentService {
  ...
  protected convertDateFromClient<T extends IPayment | NewPayment | PartialUpdatePayment>(payment: T): RestOf<T> {
    return {
      ...payment,
      datePaid: payment.datePaid?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPayment: RestPayment): IPayment {
    return {
      ...restPayment,
      datePaid: restPayment.datePaid ? dayjs(restPayment.datePaid) : undefined,
    };
  }
